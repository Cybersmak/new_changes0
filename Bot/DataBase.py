...
class Database:
    ...
    def set_F_Name(self,User_ID,Name):
        with self.connection:
    ...
    def get_L_Name(self,User_ID):
        with self.connection:
            result = self.cursor.execute("SELECT `L_Name` FROM `users` WHERE `User_ID` = ?",(User_ID,)).fetchall()
            for row in result:
                Name = str(row[0])
            return Name
    def set_L_Name(self,User_ID,L_Name):
        with self.connection:
            return self.cursor.execute("UPDATE `users` SET `L_Name` = ? WHERE `User_ID` = ?",(L_Name,User_ID,))
